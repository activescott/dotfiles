...
- [External comms voice](rules/external-comms-voice.md) — when drafting Slack/PR-comment/email messages Scott will send as himself: observational first person, no emoji/FYI/bold headers, one ask as a question, cut the forensics and unsolicited advice, keep every technical specific
...
- [Handoff file](rules/handoff-file.md) — maintain `docs/specs/handoff.md` listing spec-worthy in-progress work only (never completed work), one line per item linking to its spec dir; add line when starting; on completion write final status/caveats to the spec's summary.md then remove the line entirely if no follow-ups remain; commit per repo's spec-doc practice
